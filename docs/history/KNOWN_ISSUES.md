...
## 0. Phase 3A limitations and honest caveats

Verified automatically by `tests/phase_3a_smoke_test.tscn` (34 checks, all
passing headless on Godot 4.7.1). What that run does **not** cover:

- **Visual placement.** The Tavern Hand's start position and idle marker in
  `main.tscn` were chosen from the navigation polygon's coordinates, not by
  looking at the room. The worker paths correctly and reaches the bar, the
  seats and its idle point in headless testing, but the spot may still look
  wrong next to the furniture. Nudge `Entities/TavernHand` in the editor if so.
- **Communication UI layout.** Toasts, the alert panel and the speaker panel
  were built and positioned without a visible viewport. They are readable at
  1280×720 by construction, but no human has looked at them.
- **One worker only.** The framework is built for several — capabilities,
  atomic claims and per-slot exclusivity are all in place — but only one
  Tavern Hand has ever been in the scene at once, so multi-worker contention
  is reasoned about rather than observed.
- **Wrong drink in a service slot.** Handled by never matching it: the worker
  only ever collects a slot whose `item_id` equals the order. A drink nobody
  ordered simply sits there, which is correct but means the player gets no
  feedback that they poured the wrong thing.
- **Pending deliveries are not counted** in "replacement stock available".
  The alert reports what is physically in storage now. Ordered-but-undelivered
  stock would need `OrderManager` to expose it, which was out of scope.
- **A task completed by the player still completes.** If the player cleans a
  seat a worker had claimed, the task is recorded as COMPLETED rather than
  CANCELLED, because the world requirement genuinely was met. The worker is
  *not* credited for it — `did_perform_work()` keeps the per-worker counters
  honest — but the board's completion total includes it.
- **Broken glass restarts the cycle.** Cleaning that triggers the complication
  is treated as a successful action, and the new mess produces a new task. That
  is intended, but it means one seat can generate two cleaning tasks in a row.

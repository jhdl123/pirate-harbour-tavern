...
func can_claim(worker: Node, task: TavernTask) -> bool:
	...
	var storage := task.get_source() as StockStorage
	...
	if carrier.is_carrying():
		return false
	return storage.count_item(task.required_definition.item_id) > 0 and station.current_servings < station.maximum_servings
...
func get_next_step(worker: Node, task: TavernTask) -> StaffTaskStep:
	...
	if storage.count_item(task.required_definition.item_id) <= 0:
...
func perform_action(worker: Node, task: TavernTask) -> ActionResult:
	...
	match _pending_action:
		&"collect_stock":
			...
			if storage == null:
				return ActionResult.FAILED
			var result := storage.take_one(task.required_definition.item_id, carrier)
